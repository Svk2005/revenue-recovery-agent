...
expected = sum(float(r["expected_recovered_value_inr"] or 0) for r in rows)
measured = sum(float(r["measured_recovered_value_inr"] or 0) for r in rows)
outcomes = Counter(r["send_outcome"] for r in rows)
reasons = Counter(r["failure_reason"] for r in rows)
confirmed = sum(1 for r in rows if r["payment_status"] == "confirmed")
pending = sum(1 for r in rows if r["payment_status"] == "pending")
...
lines.append(f"- **MEASURED value recovered (confirmed payments only):** "
             f"Rs {measured:,.2f} ({100*measured/total_value:.1f}% of batch)")
lines.append(f"- Expected value recovered (formula estimate, for comparison): "
             f"Rs {expected:,.2f}")
lines.append(f"- Confirmed payments: {confirmed}  ·  Pending confirmation: {pending}")
lines.append("")
lines.append("## Outcomes")
for outcome, count in outcomes.most_common():
    lines.append(f"- {outcome}: {count}")
lines.append("")
lines.append("## Failure reasons diagnosed (real Razorpay error taxonomy)")
